defmodule BatchOperationOptionsTest do
  ...
  describe "new/5" do
    ...
    test "creates valid batch operation options with async mode" do
      ...
      assert {:ok, opts} =
               BatchOperationOptions.new(0, handle_batch, on_complete, :async, health_checkers)
    ...
    test "returns error when handle_batch is nil" do
      ...
      assert {:error, :invalid_handle_batch} =
               BatchOperationOptions.new(0, nil, nil, :sync, health_checkers)
    end
    ...
    test "returns error when handle_batch is not a function" do
      ...
      assert {:error, :invalid_handle_batch} =
               BatchOperationOptions.new(0, "not a function", nil, :sync, health_checkers)
    end
    ...
    test "returns error when mode is invalid" do
      ...
      assert {:error, :invalid_mode} =
               BatchOperationOptions.new(0, handle_batch, nil, :invalid, health_checkers)
    end
    ...
    test "returns error when health_checkers is nil" do
      ...
      assert {:error, :invalid_health_checkers} =
               BatchOperationOptions.new(0, handle_batch, nil, :sync, nil)
    end
    ...
    test "returns error when health_checkers is empty list" do
      ...
      assert {:error, :invalid_health_checkers} =
               BatchOperationOptions.new(0, handle_batch, nil, :sync, [])
    end
