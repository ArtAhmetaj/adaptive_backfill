defmodule SingleOperationOptionsTest do
  ...
  describe "new/4" do
    ...
    test "creates valid single operation options with async mode" do
      ...
      assert {:ok, opts} =
               SingleOperationOptions.new(handle, on_complete, :async, health_checkers)
    ...
    test "returns error when handle is nil" do
      ...
      assert {:error, :invalid_handle} =
               SingleOperationOptions.new(nil, nil, :sync, health_checkers)
    end
    ...
    test "returns error when handle is not a function" do
      ...
      assert {:error, :invalid_handle} =
               SingleOperationOptions.new("not a function", nil, :sync, health_checkers)
    end
    ...
    test "returns error when on_complete has wrong arity" do
      ...
      assert {:error, :invalid_on_complete} =
               SingleOperationOptions.new(handle, on_complete, :sync, health_checkers)
    end
    ...
    test "returns error when mode is invalid" do
      ...
      assert {:error, :invalid_mode} =
               SingleOperationOptions.new(handle, nil, :invalid, health_checkers)
    end

    test "returns error when health_checkers is nil" do
      handle = fn _health_check -> {:ok, :result} end

      assert {:error, :invalid_health_checkers} =
               SingleOperationOptions.new(handle, nil, :sync, nil)
    end
    ...
    test "returns error when health_checkers is empty list" do
      ...
      assert {:error, :invalid_health_checkers} =
               SingleOperationOptions.new(handle, nil, :sync, [])
    end
